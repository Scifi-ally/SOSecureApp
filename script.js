...
function showPopup(message, isSosAlert = false, sosData = null) {
...
function initMap() {
    ...
    directionsRenderer = new google.maps.DirectionsRenderer({
        map: map,
        polylineOptions: { strokeColor: "#ffffff", strokeWeight: 4, strokeOpacity: 1.0 }
    });
...
function navigateToSosLocation(lat, lng) {
    if (!isMapInitialized || !map || !window.google || !window.google.maps) {
        showPopup('Map not initialized yet.');
        return;
    }
